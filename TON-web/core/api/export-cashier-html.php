...
if (!RateLimiter::check('export_cashier_html', 10, 60)) {
...
if (json_last_error() !== JSON_ERROR_NONE) {
    error_log("JSON decode error in export-cashier-html.php: " . json_last_error_msg());
    http_response_code(500);
...
if (!empty($webhook_url)) {
    ...
    ");
    $stmt->bind_param("ss", $webhook_url, $user_id);
    $stmt->execute();
...
$filename = 'cashier_' . $cashier_id . '_report_' . date('Y-m-d_His') . '.html';
...
<head>
    ...
    <title>Отчет по кассе <?php echo htmlspecialchars($cashier['name'] ?? 'Без названия'); ?></title>
...
<body>
    <div class="container">
        <h1>Отчет по кассе: <?php echo htmlspecialchars($cashier['name'] ?? 'Без названия'); ?></h1>
        <div class="subtitle">Сгенерировано: <?php echo date('d.m.Y H:i:s'); ?></div>
        
        <div class="info-grid">
            <div class="info-card">
                <div class="info-card-label">ID кассы</div>
                <div class="info-card-value"><?php echo $cashier_id; ?></div>
            </div>
            <div class="info-card">
                <div class="info-card-label">Статус</div>
                <div class="info-card-value"><?php echo ($cashier['status'] ?? 'inactive') === 'active' ? 'Активна' : 'Неактивна'; ?></div>
            </div>
            <div class="info-card">
                <div class="info-card-label">Валюта</div>
                <div class="info-card-value"><?php echo strtoupper($cashier['currency'] ?? 'TON'); ?></div>
            </div>
            <div class="info-card">
            ...
            <div class="info-card">
                ...
                <div class="info-card-value"><?php echo number_format($total_amount, 2, '.', ' '); ?> <?php echo strtoupper($cashier['currency'] ?? 'TON'); ?></div>
            </div>
            <div class="info-card">
                <div class="info-card-label">Успешная сумма</div>
                <div class="info-card-value"><?php echo number_format($successful_amount, 2, '.', ' '); ?> <?php echo strtoupper($cashier['currency'] ?? 'TON'); ?></div>
            </div>
            <div class="info-card">
                <div class="info-card-label">Текущий баланс</div>
                <div class="info-card-value"><?php echo number_format(floatval($cashier['balance'] ?? 0), 2, '.', ' '); ?> <?php echo strtoupper($cashier['currency'] ?? 'TON'); ?></div>
            </div>
        </div>

        <?php if (count($chart_labels) > 0): ?>
        <div class="chart-container">
            <div class="chart-title">Динамика транзакций по дням</div>
        ...
        <table>
            ...
            <tbody>
                ...
                foreach ($all_transactions as $tx): 
                    $status = $tx['status'] ?? 'other';
                    $status_class = 'status-' . ($status === 'success' ? 'success' : ($status === 'pending' ? 'pending' : 'failed'));
                    $status_text = $status === 'success' ? 'Успешно' : ($status === 'pending' ? 'Ожидание' : 'Отменено');
                ?>
                <tr>
                    <td><?php echo $tx['id']; ?></td>
                    <td><?php echo date('d.m.Y H:i:s', strtotime($tx['time_recorded'])); ?></td>
                    <td><?php echo number_format($tx['price'], 2, '.', ' '); ?></td>
                    <td><?php echo strtoupper($tx['currency'] ?? 'TON'); ?></td>
                    <td class="<?php echo $status_class; ?>"><?php echo $status_text; ?></td>
                    <td style="font-family: monospace; font-size: 12px;"><?php echo htmlspecialchars($tx['hash'] ?? ''); ?></td>
                </tr>
                <?php endforeach; ?>
            </tbody>
        </table>
    ...
    <script>
        ...
        new Chart(ctx2, {
            ...
            data: {
                labels: <?php echo json_encode($chart_labels); ?>,
                datasets: [{
                    label: 'Накопительная сумма (<?php echo strtoupper($cashier['currency'] ?? 'TON'); ?>)',
